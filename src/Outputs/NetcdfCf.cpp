...
OutputNetcdfCf::OutputNetcdfCf(const Options& iOptions, const Data& iData) : Output(iOptions, iData),
      mDimOffset("time"),
      mDimThreshold("threshold"),
      mVarLat("latitude"),
      mVarLon("longitude"),
      mVarElev("altitude"),
      mNumX(Global::MV)
{
   ...
   //! Should locations be re-gridded? If so, how long is the first dimension?
   iOptions.getValue("numX", mNumX);
      ...
      for(int v = 0; v < variables.size(); v++) {
         ...
         for(int initI = 0; initI < inits.size(); initI++) {
            ...
            for(int d = 0; d < dates.size(); d++) {
               ...
               // Set up dimensions
               NcDim* dimOffset    = ncfile.add_dim(mDimOffset.c_str());
               ...
               if(getThresholds().size() > 0)
                  dimThreshold   = ncfile.add_dim(mDimThreshold.c_str(), getThresholds().size());
               if(getCdfs().size() > 0)
               ...
               NcVar* varLat      = ncfile.add_var(mVarLat.c_str(),ncFloat, dimLocation);
               NcVar* varLon      = ncfile.add_var(mVarLon.c_str(),ncFloat, dimLocation);
               NcVar* varElev     = ncfile.add_var(mVarElev.c_str(),ncFloat, dimLocation);
               ...
               NcVar* varDimOffset    = ncfile.add_var(mVarOffset.c_str(), ncFloat, dimOffset);
               NcVar* varDimThreshold = NULL;
               if(dimThreshold != NULL)
                  varDimThreshold = ncfile.add_var(mVarThreshold.c_str(), ncFloat, dimThreshold);

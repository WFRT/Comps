...
OutputNetcdf::OutputNetcdf(const Options& iOptions, const Data& iData, const Configuration& iConfiguration) : Output(iOptions, iData, iConfiguration) {}

void OutputNetcdf::write() const {
   ...
   getAllDates(mEnsembles, dates);
   getAllVariables(mEnsembles, variables);
   ...
   for(int v = 0; v < variables.size(); v++) {
      ...
      for(int d = 0; d < dates.size(); d++) {
         ...
         assert(numMembers > 0);
         // Set up file
         std::string filename = getFilename(date, variable);
         ...
         for(int i = 0; i < mEnsembles.size(); i++) {
            Deterministic det = mDeterministics[i];
...
std::string OutputNetcdf::getFilename(int iDate, std::string iVariable) const {
   // Clear file
   std::stringstream ss;
   ss << getOutputDirectory() << iDate << "_" << iVariable << "_" << mConfiguration.getName() << ".nc";
   return ss.str();
}

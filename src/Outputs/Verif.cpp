...
OutputVerif::OutputVerif(const Options& iOptions, const Data& iData, const Configuration& iConfiguration) : Output(iOptions, iData, iConfiguration) {}

void OutputVerif::write() const {
   ...
   for(int v = 0; v < variables.size(); v++) {
      ...
      for(int d = 0; d < dates.size(); d++) {
         ...
         // Set up file
         std::string filename = getFilename(variable);
         ...
         for(int i = 0; i < (int) mScores.size(); i++) {
            if(mScores[i].getVariable() == variable) {
...
std::string OutputVerif::getFilename(std::string iVariable) const {
   // Clear file
   std::stringstream ss;
   ss << getOutputDirectory() << iVariable << "_" << mConfiguration.getName() << ".nc";
   return ss.str();
}

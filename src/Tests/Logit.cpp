...
namespace {
   ...
   /*
   // With default parameters, P should always be 0.5
   TEST_F(LogitTest, simpleP) {
      std::vector<std::string> types;
      types.push_back("useConst");
      types.push_back("useMean");
      types.push_back("useFrac");
      types.push_back("useConst useMean");
      types.push_back("useConst useFrac");
      types.push_back("useMean useFrac");
      types.push_back("useConst useMean useFrac");

      for(int i = 0; i < (int) types.size(); i++) {
         std::stringstream ss;
         ss << "tag=test class=DiscreteLogit " << types[i];
         Discrete* logit = Discrete::getScheme(Options(ss.str()), *mData);

         Parameters par;
         logit->getDefaultParameters(par);
         float p = logit->getP(mEnsemble, par);
         EXPECT_FLOAT_EQ(0.5, p);
   ...
   */
   ...
   TEST_F(LogitTest, useConst) {
      //Discrete* logit = Discrete::getScheme(Options("tag=test class=DiscreteLogit useConst efold=2"), *mData);
      Discrete* logit = Discrete::getScheme(Options("tag=test class=DiscreteLogit useConst efold=2 estimator=test.mle x=0"), *mData);

      // Percentage of obs that are 0
      ...
      for(int i = 0; i < sizeof(fracs)/sizeof(float); i++) {
         ...
         std::vector<Obs> obs;
         for(int k = 0; k < N; k++) {
            if((float) k/N < fracs[i]) {
               obs.push_back(Obs(0, mDate, 0, mVariable, mLocation));
            }
            else {
               obs.push_back(Obs(1, mDate, 0, mVariable, mLocation));
            }
         }
   ...
   /*
   TEST_F(LogitTest, useMean) {
      Discrete* logit = Discrete::getScheme(Options("tag=test class=DiscreteLogit useConst useMean efold=2"), *mData);
   ...
   TEST_F(LogitTest, all) {
      Discrete* logit = Discrete::getScheme(Options("tag=test class=DiscreteLogit useConst useMean useFrac efold=2 estimator=test.mle"), *mData);
